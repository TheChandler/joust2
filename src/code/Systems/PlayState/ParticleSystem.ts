...
export class ParticleSystem {
    ...
    add(x, y, xvel, yvel, lifetime) {
        let i = this.lifetime.findIndex(n => n == 0);
        this.xvel[i] = xvel;
        this.yvel[i] = yvel;
        // this.xvel[i] = 0;
        // this.yvel[i] = 0;
    ...
    update() {
        for (let i = 0; i < ParticleSystem.ARRAY_SIZE; i++) {
            if (this.lifetime[i]) {
                ...
                // this.lifetime[i] = (this.xpositions[i] >> 4) > canvas.width ? 0 : this.lifetime[i] - 1;
                // if (this.lifetime[i] == 0) {
                //     console.log("Dead", i, this.xpositions[i] >> 4, this.ypositions[i] >> 4)
                // }
